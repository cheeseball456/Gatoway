...
| AD-4 | Both listeners bind to loopback only (`127.0.0.1`/`::1`), never `0.0.0.0` | Decided | Satisfies NFR 3.3 (local-only communication) at the network layer; an OS-level refusal of non-loopback connections holds regardless of firewall state, unlike an application-layer check | None seriously considered — this is a baseline, not a contested trade-off |
