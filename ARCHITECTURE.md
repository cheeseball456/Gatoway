...
| AD-9 | **(Revised v1.7, superseding v1.6.)** The Stream Deck plugin reports the connected device's **fixed physical layout** — its button grid dimensions and dial count, derived once from the Elgato SDK's `Device.size`/`Device.type` (a static per-model fact, not a function of what's currently placed) — to Gatoway core, at its own registration and again whenever the connected device itself changes (connected/disconnected/swapped). This directly yields the ordered position lists AD-6/AD-8's labels are derived from. Gatoway core holds this as in-memory-only current state and forwards the resulting counts to each application plugin at that plugin's own connection time, and again every time that plugin reports gaining focus | Decided (amended v1.7) | Resolves `REQUIREMENTS.md` FR-007, corrected after QA-020 (found during live `/verify`): v1.6 derived capacity from *which positions currently held a placed generic action* (`Device.actions`, reacting to `willAppear`/`willDisappear`), which meant the same physical button could mean a different ordinal slot depending on unrelated placement changes — the user needs a *stable* address per physical position, not a live-shifting one. Deriving purely from `Device.size`/`Device.type` is both more correct (matches the actual, fixed hardware) and simpler to implement (no more reacting to individual action-placement events — only to the device itself connecting/disconnecting, a strictly rarer event). Push-based and event-driven, matching this project's existing self-reported style (AD-7) rather than introducing polling. Refreshing on every focus change (not only once at connection) remains unchanged from v1.6, so the design still does not preclude a future where multiple physical Stream Decks exist, or a device is connected/disconnected while plugins remain connected (`REQUIREMENTS.md` §2.3) | v1.6's placement-derived capacity (`Device.actions`-based) — superseded per QA-020; polling Gatoway core for capacity on demand — rejected as inconsistent with the project's established event-driven philosophy; sending capacity only once at connection — rejected because it would go stale if the device itself changed mid-session |
...
derived addressing. It should be reviewed before change proposals are drafted against
it.*
