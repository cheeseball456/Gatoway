...
- [x] 8.5 Unit + integration tests for resolution: ordinal index resolved correctly; underflow (input event's index has no content entry) safely dropped; position not in `device_capacity` safely dropped; overflow (more declared content than capacity) never causes an out-of-range render
