...
- [x] 6.5 Manually verify on physical Stream Deck+ hardware, using test-double app connections (no real Lightroom/xDesign yet): placing the generic actions, seeing the idle appearance by default, simulating a test app gaining focus and seeing the display update accordingly, pressing a bound key and confirming the resolved command is sent, and confirming the idle appearance returns when the test app blurs or disconnects — **completed via `/verify` with the user and real Stream Deck+ hardware** (see `QA_REPORT.md`'s "Interactive Verification Session" for this change). Confirmed live: generic Key/Dial actions render correctly once placed; idle appearance shows by default; a test-double app registering and reporting focus drives real-time rendering; keypad press and dial rotation both correctly resolve to `command` messages; a live `capability_update` with no physical input immediately updates the display; and both explicit blur and disconnect-while-focused correctly revert to idle with the icon properly reset (`icon:null` confirmed in Gatoway core's actual log payloads). One Minor finding surfaced and was resolved by `doc-writer` (QA-012, capability-label length guidance).
...
- [x] 7.1 Add the `capability_update` message payload type (`capabilityId`, optional `icon`/`label`/`state`)
- [x] 7.2 Change `render_update`'s `icon` field to accept `string | null | undefined` (`null` = explicit reset to manifest default, `undefined`/omitted = unchanged)
- [x] 7.3 Change `LayoutResolver.resolve()` to return a capability **id** (`string | null`), not a full `Capability` object; update the test-fixture resolver and its own tests accordingly
- [x] 7.4 Add a live capability lookup (given a connection and a capability id, find that capability in the connection's own stored `capabilities` array)
- [x] 7.5 Update `ProfileRouter`'s bound-layout sweep to resolve id → live capability via 7.4, rather than using a resolver-embedded snapshot
- [x] 7.6 Update `ProfileRouter`'s idle sweep to send `icon: null` explicitly, not omit `icon`
- [x] 7.7 Handle incoming `capability_update`: reject/no-op if the capability id isn't in the sender's own declared capabilities; otherwise sparse-merge the update into the stored record
