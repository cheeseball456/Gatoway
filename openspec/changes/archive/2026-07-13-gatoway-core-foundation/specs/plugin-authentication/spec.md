...
#### Scenario: Allowlisted origin accepted
- **WHEN** a WebSocket upgrade request's `Origin` header matches an allowlisted extension origin
- **THEN** Gatoway core completes the upgrade and marks the connection as authenticated

#### Scenario: Non-allowlisted origin refused
- **WHEN** a WebSocket upgrade request's `Origin` header does not match any allowlisted origin
