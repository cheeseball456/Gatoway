...
- [x] 5.1 In `genericKeyRenderer.ts`/the generic Key action's `onWillAppear` handling: when no remembered render state exists yet for a position, immediately apply the manifest's declared default label/icon locally, before/independent of anything from Gatoway core
- [x] 5.2 Do the same in `genericDialRenderer.ts`/the generic Dial action's `onWillAppear` handling
- [x] 5.3 Confirm a subsequent real `render_update` from Gatoway core still overrides the local baseline exactly as before (no regression to existing `render_update` handling)
- [x] 5.4 Add unit tests: no remembered state → local baseline applied without any Gatoway core message; local baseline is correctly overridden once a `render_update` arrives
- [x] 5.5 Manually re-verify on physical hardware: repeat the missing-config + plugin-restart scenario from `/verify` and confirm the dial (and keys) now show the correct default baseline immediately, with no stuck/uninitialized appearance — **completed via `/verify` with the user**: reproduced the exact original failure (config removed, plugin restarted), confirmed the dial now shows "Gatoway" correctly instead of getting stuck on "Dial"; restored the config and confirmed all 4 positions correct afterward
