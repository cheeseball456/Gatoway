...
**D1 — New message types.** Four additions to the existing envelope (`{ type, connectionId?, payload }`), no changes to framing or existing types (amended: this originally listed three types, but omitted the app-facing message the `profile-routing` spec itself requires — "send a corresponding command to the focused connection" — a gap only surfaced during implementation and closed here):
